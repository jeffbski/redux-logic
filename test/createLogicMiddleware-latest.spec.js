...
describe('createLogicMiddleware-latest', () => {
  ...
  describe('[logicA] latest=falsey validate allow next async', () => {
    let mw;
    let logicA;
    let next;
    let dispatch;
    const actionFoo1 = { type: 'FOO', id: 1 };
    const actionFoo2 = { type: 'FOO', id: 2 };
    const actionResultFoo1 = { type: 'FOO', id: 1, trans: ['a'] };
    const actionResultFoo2 = { type: 'FOO', id: 2, trans: ['a'] };
    beforeEach(done => {
      next = expect.createSpy().andCall(nextCb);
      let nextCount = 0;
      function nextCb() {
    ...
    it('passes both actionFoo1 and actionFoo2', () => {
      expect(next.calls.length).toBe(2);
      expect(next.calls[0].arguments[0]).toEqual(actionResultFoo1);
      expect(next.calls[1].arguments[0]).toEqual(actionResultFoo2);
    });
  ...
  describe('[logicA] latest=falsey validate reject next async', () => {
    let mw;
    let logicA;
    let next;
    let dispatch;
